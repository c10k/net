...
TEST(Socket, bind)
{
	...
	EXPECT_NO_THROW(ipv4Socket.bind(
	...
	EXPECT_NO_THROW(sock_liar.bind([](AddrIPv4 &s) { return 5; }));
	EXPECT_ANY_THROW(sock_liar.connect("127.0.0.1", 15000));


	Socket sock_liar_ipv4(Domain::IPv4, Type::TCP);
	EXPECT_THROW(sock_liar_ipv4.bind([](AddrIPv6 &s) {
	...
	EXPECT_THROW(sock_bad_addr6.bind([](AddrIPv6 &s) { return 0; }),
	             std::invalid_argument);


	Socket sock_bad_addr62(Domain::IPv6, Type::TCP);
	EXPECT_THROW(sock_bad_addr62.bind([](AddrIPv6 &s) { return -1; }),
	             std::runtime_error);
